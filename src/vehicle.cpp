...
vector<Vehicle> Vehicle::choose_next_state(map<int, vector<Vehicle>> predictions) {
  ...
  */
  ...
  std::vector< vector<Vehicle> > final_trajectories;

  for (std::vector<string>::iterator it = possible_successor_states.begin(); it != possible_successor_states.end(); ++it) {

    vector<Vehicle> trajectory_for_state;
    trajectory_for_state = generate_trajectory(*it, predictions);

    if (trajectory_for_state.size() != 0) {
      float cost_for_state = calculate_cost(*this , predictions, trajectory_for_state);
      //cout << cost_for_state << endl;
  ...
  return final_trajectories[min_cost_ix];
}
...
vector<string> Vehicle::successor_states() {
  ...
  */
  ...
  if(state.compare("KL") == 0) {
    ...
    if (lane != 0) {
      // lane 0 is the left most lane in the simulator
      states.push_back("PLCL");
      states.push_back("LCL");
    } // if
  } else if (state.compare("PLCR") == 0) {
    if (lane != lanes_available - 1) {
      // lane 2 is the right most lane
      states.push_back("PLCR");
